...
// Trust is the struct that implements the handler interface for the trust resource
type Trust struct {
...
func (c *Trust) Routes() []types.RouteDetails {
	return []types.RouteDetails{
		...
		types.RouteDetails{
			Enabled: true, Path: "/:userID", Verb: "GET",
			Handler: c.GetSingle,
			Scopes:  []string{},
		},
		types.RouteDetails{
			Enabled: true, Path: "/:userID", Verb: "PATCH",
			Handler: c.Update,
			Scopes:  []string{"trust:manage"},
		},
		types.RouteDetails{
			Enabled: true, Path: "/:userID", Verb: "POST",
			Handler: c.Create,
			Scopes:  []string{"trust:create"},
		},
		types.RouteDetails{
			Enabled: true, Path: "/:userID", Verb: "DELETE",
			Handler: c.Delete,
			Scopes:  []string{"trust:manage"},
		},
	}
...
func (c *Trust) ReturnOne(filter map[string]interface{}) (ResponseSchema, error) {
...
func (c *Trust) GetAll(ctx *gin.Context) {
...
func (c *Trust) GetSingle(ctx *gin.Context) {
	token := html.EscapeString(ctx.Param("token"))
	userID := html.EscapeString(ctx.Param("userID"))
	filter := map[string]interface{}{"token": token, "userId": userID}
...
func (c *Trust) Create(ctx *gin.Context) {
	...
	filter := map[string]interface{}{
		"token": createVals.Token, "userId": createVals.UserID}
	res, err := c.ReturnOne(filter)
	...
	createData, err := util.ValidateAndMap(
		ctx.Request.Body, "/trust/createSchema.json", createVals)
...
func (c *Trust) Update(ctx *gin.Context) {
	...
	updateData, err := util.ValidateAndMap(
		ctx.Request.Body, "/trust/schema.json", updateVals)
...
func (c *Trust) Delete(ctx *gin.Context) {
	token := html.EscapeString(ctx.Param("token"))
	userID := html.EscapeString(ctx.Param("userID"))
	filter := map[string]interface{}{"token": token, "userId": userID}
